...
export function getHtmlTemplateCode(): string {
  return `<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8">
  ...
  <meta property="og:url" content="https://blackmarket.sourcing/">
  ...
  <meta property="og:image" content="https://images.unsplash.com/photo-1548883354-7622d03aca27?auto=format&fit=crop&q=80&w=1200">
...
<body class="min-h-screen flex flex-col font-sans">
  ...
  <header class="bg-[#0d0d14] border-b border-brand-red/30 py-4 shadow-md sticky top-0 z-30">
    <div class="max-w-6xl mx-auto px-4 flex flex-col sm:flex-row justify-between items-center gap-4">
      ...
      <a id="wa-link" href="https://wa.me/2250707070707" target="_blank" rel="noopener noreferrer" class="bg-brand-red hover:bg-red-600 text-white text-xs px-4 py-2 rounded-xl font-bold font-mono transition-all">
        💬 WHATSAPP DIRECT
      </a>
    </div>
  </header>
        ...
        <div class="space-y-4 flex flex-col justify-between">
          <div class="space-y-3">
            ...
            <div class="space-y-1">
              ...
              <p id="modal-description" class="text-xs text-zinc-300 leading-relaxed bg-black/40 p-3 rounded-lg border border-zinc-900 max-h-[150px] overflow-y-auto"></p>
            </div>
          </div>
  ...
  <script>
    ...
    const CONFIG = { phoneNumber: "2250707070707", currency: "XOF" /* ou EUR */ };
    ...
    function renderCategories() {
      ...
      container.innerHTML = uniqueCats.map(cat => {
        ...
        return '<button onclick="filterCategory(\\'' + escapeHtml(cat) + '\\')" class="' + base + ' text-[10px] px-4 py-1.5 rounded-full font-extrabold font-mono transition-all uppercase tracking-widest">' + escapeHtml(cat) + "</button>";
      }).join("");
    }
          ...
          '<div onclick="openDetailsModal(' + masterIndex + ')" class="bg-brand-card rounded-3xl overflow-hidden border border-zinc-800 flex flex-col h-full relative group hover:border-brand-red/40 transition-all duration-300 cursor-pointer">' +
            ...
            '<div class="p-6 flex flex-col flex-1 justify-between gap-4">' +
              '<div class="space-y-1.5">' +
                ...
                '<p class="text-xs text-zinc-400 leading-relaxed line-clamp-3">' + escapeHtml(p.description) + "</p>" +
              "</div>" +
              '<div class="flex items-center justify-between pt-2 border-t border-zinc-900">' +
                ...
                '<a href="' + waUrl + '" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation();" class="bg-brand-red text-white font-bold text-[11px] px-3.5 py-2 rounded-xl transition-all font-mono">COMMANDER</a>' +
              "</div>" +
            "</div>" +
          "</div>";
      });
