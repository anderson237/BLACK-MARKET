...
<body class="min-h-screen flex flex-col font-sans">
  ...
  <div id="details-modal" class="fixed inset-0 bg-black/95 backdrop-blur-md flex items-center justify-center p-4 z-50 hidden overflow-y-auto">
    <div class="bg-[#15151e] rounded-3xl shadow-2xl max-w-2xl w-full overflow-hidden border border-brand-red/30 text-slate-200 flex flex-col my-8">
      ...
      <div class="p-6 overflow-y-auto max-h-[75vh] grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="relative aspect-square rounded-2xl overflow-hidden bg-zinc-950 border border-zinc-800 flex items-center justify-center" id="modal-carousel">
          <img id="modal-image" src="" alt="" crossorigin="anonymous" class="w-full h-full object-cover">
          <button onclick="bmModalMove(-1)" id="modal-prev" class="hidden absolute left-2 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-brand-red/80 text-white w-8 h-8 rounded-full text-lg font-black leading-none border border-white/20 transition-colors z-10">‹</button>
          <button onclick="bmModalMove(1)" id="modal-next" class="hidden absolute right-2 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-brand-red/80 text-white w-8 h-8 rounded-full text-lg font-black leading-none border border-white/20 transition-colors z-10">›</button>
          <div id="modal-dots" class="hidden absolute bottom-2 left-0 right-0 flex justify-center gap-1.5 z-10"></div>
        </div>
  ...
  <script>
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(data) && data.length > 0) {
            const changed = JSON.stringify(data) !== JSON.stringify(PRODUCTS);
            PRODUCTS = data;
            if (!catalogLoaded || changed) {
              const firstLoad = !catalogLoaded;
              catalogLoaded = true;
              renderCategories();
              renderGrid(PRODUCTS);
              if (firstLoad) handleDeepLink();
              // Refresh an open details modal so admin edits show up live.
              if (currentModalId) {
                for (var i = 0; i < PRODUCTS.length; i++) {
                  if (PRODUCTS[i].id === currentModalId) { openDetailsModal(i); break; }
                }
              }
            }
          }
    ...
    // Auto-refresh: poll the live catalog so admin changes appear without reload.
    ...
    function renderGrid(itemsToRender) {
      ...
      itemsToRender.forEach(p => {
        ...
        // Prefer the live (uploaded / AI-generated, already watermarked) image;
        // otherwise use the static pre-watermarked copy baked at build time.
        const isLiveImg = String(p.imageUrl || "").charAt(0) === "/";
        const cardImg = isLiveImg ? p.imageUrl : localImg;
        const cardFallback = isLiveImg ? localImg : (p.imageUrl || "");
    ...
    function openDetailsModal(index) {
      ...
      // Build a media carousel: main image + gallery photos + optional video.
      var media = [{ type: "image", src: p.imageUrl || "" }];
      (p.gallery || []).forEach(function (u) { if (u) media.push({ type: "image", src: u }); });
      if (p.videoUrl) media.push({ type: "video", src: p.videoUrl });
      window.__bmMedia = media;
      window.__bmMediaIndex = 0;

      function renderModalSlide(idx) {
        var m = window.__bmMedia[Math.min(idx, window.__bmMedia.length - 1)];
        if (!m) return;
        window.__bmMediaIndex = Math.min(idx, window.__bmMedia.length - 1);
        ...
        var multi = window.__bmMedia.length > 1;
        if (multi) {
          dots.innerHTML = window.__bmMedia.map(function (_, i) {
            return '<span class="w-1.5 h-1.5 rounded-full ' + (i === window.__bmMediaIndex ? "bg-brand-red" : "bg-white/40") + '" style="display:inline-block"></span>';
          }).join("");
        ...
        if (m.type === "video") {
          ...
          modalImg.outerHTML = '<video id="modal-image" src="' + escapeHtml(m.src) + '" controls playsinline preload="metadata" class="w-full h-full object-cover bg-black"></video>';
          return;
        }
        if (modalImg.tagName === "VIDEO") {
          var holder = document.createElement("img");
          holder.id = "modal-image";
          holder.className = "w-full h-full object-cover";
          holder.crossOrigin = "anonymous";
          modalImg.parentNode.replaceChild(holder, modalImg);
          modalImg = document.getElementById("modal-image");
        }
        var isLive = String(m.src || "").charAt(0) === "/";
        modalImg.src = isLive ? m.src : ("img/" + encodeURIComponent(p.id) + ".jpg");
        ...
        if (!isLive) {
          modalImg.crossOrigin = "anonymous";
          modalImg.onload = function () { watermarkImage(modalImg); };
        }
      }
      window.bmModalMove = function (d) {
        if (!window.__bmMedia || !window.__bmMedia.length) return;
        renderModalSlide((window.__bmMediaIndex + d + window.__bmMedia.length) % window.__bmMedia.length);
      };
