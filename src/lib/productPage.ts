...
export function getProductPageHtml(p: Product, baseUrl: string, phoneNumber: string): string {
  ...
  const features = (p.features || [])
    ...
    .join("\n");
  const message =
    "Bonjour, je souhaite commander le produit [" + String(p.title || "").toUpperCase() + "] au prix de [" + priceXof + " F CFA]. Voici le produit : " + pageUrl;
...
${features}
            ...
            <p class="text-xs text-zinc-400 leading-relaxed bg-black/40 p-3 rounded-lg border border-zinc-900">${description}</p>
          </div>
