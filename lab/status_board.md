...
| ST-012 | Chat commandes / précommandes (client ↔ admin) | LIVE | ✅ Déployé | Threads de discussion : `pre:<userId>` (précommandes/panier) + `ord:<orderId>` (commandes) en blob `bm-chat`. Client écrit depuis `/compte` (onglets Précommandes & Commandes), admin répond depuis `/admin/carts` (panier) & `/admin/orders` (modal détail). Badges non-lus des deux côtés (sidebar admin + badges par thread). **Conservation des messages** à la confirmation de précommande (migration pre→ord). **Email admin** (Resend, util partagé `server/utils/email.ts`) à la confirmation d'une commande. **Temps réel** : SSE (`/api/chat/events` + `/api/admin/chat/events`, push instantané via `server/utils/realtime.ts` + `composables/useSse.ts`) + **fallback poll 2 s** pour couvrir la séparation d'instances serverless ; ChatPanel marque lu + scrolle automatiquement à l'arrivée de nouveaux messages. Test e2e prod OK (push SSE vérifié, ping 15 s OK, latence API ≤2 s). Commits `29d090c`, `96e4954`, `c3da739` ; deploys `6a76231a6a27afd09fa22933`, `6a762e1c327b4f22a3fca706`. |
