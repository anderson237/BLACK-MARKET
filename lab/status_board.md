...
| ST-020 | Extension Chrome scraper fournisseurs (Taobao/Amazon/1688 non atteignables par URL) | 10 — LIVE | ✅ Déployé `6a7a199bb3…` | Demande utilisateur : pour les sites non atteignables par URL (login/anti-bot côté serveur), disposer d'une extension Chrome qui scrappe depuis le navigateur connecté de l'admin. **Serveur** : route `POST /api/admin/import/extension` (auth `x-ext-key` temps constant OU Bearer admin, rate-limit, validation stricte, réutilise `buildDraft` — publication jamais directe) + `server/utils/extensionImport.ts` + test tsx **56/56 ✅**. **Extension** (`browser-extension/`, hors build) : MV3, hook fetch/XHR en **world MAIN**, extraction JSON/DOM en world ISOLÉ (DR_PING/DR_CAPTURE), service worker POST serveur, popup (clé + draft). **Prolongé** : traduction FR Gemini auto si titre CJK ; `EXTENSION_IMPORT_KEY` posée en prod (secret). **E2E prod ✅** (401 sans clé ; draft 129¥ → 12 255 FCFA, engine extension). Itération `lab/iterations/2026-08-10_ST-020_cycle_1.md` ; handoff retour `2026-08-10_ST-020_Squad04_to_Lab.md`. |
...
| BL-010 | **Extension Chrome scraper** (sites non-atteignables par URL : Taobao/Amazon/1688) | ✅ **Fait** (ST-020, deploy `6a7a199bb3…`) | Route `/api/admin/import/extension` + `browser-extension/` livrés ; clé `EXTENSION_IMPORT_KEY` en prod ; e2e prod ✅. Test tsx 56/56. |
...
|------|-----------|---------|
| 2026-08-10 | 6a7a199bb3c28d9ec3fdc3ef | **ST-020 + BL-010** : extension Chrome scraper fournisseurs — route `/api/admin/import/extension` (x-ext-key, buildDraft, 56 tests), dossier `browser-extension/` complet (MV3 MAIN+ISOLATED, popup), `EXTENSION_IMPORT_KEY` posée en prod, e2e prod ✅. **Gate lint réparé** : `tsconfig.json` étend `.nuxt/tsconfig.json` (nomImplicitAny off), `npm run lint` ✅ (0 erreur). |
